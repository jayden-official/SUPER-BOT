...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      else return m.reply('*[❗𝐈𝐍𝐅𝐎❗] the link /url/link is not valid, the ending of the link /url/link must be . jpg, example:: ${usedPrefix}s https://th.bing.com/th/id/OIP.U2UcnOVASrcsxRpb_2TxUQHaHa?pid=ImgDet&rs=1*')
    }
  } catch (e) {
    console.error(e)
    if (!stiker) stiker = e
    ...
    else throw '*[❗𝐈𝐍𝐅𝐎❗] An error occurred, please try again. Answer video, image or insert the link of an image termicion.jpg which will be converted into a sticker*'
  }}
handler.help = ['sfull']
