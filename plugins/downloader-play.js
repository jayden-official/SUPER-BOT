import fetch from 'node-fetch'; 
 ...
 ❏ 🧾 Type: ${yt_play[0].type} 
 ❏ 🔗 Link: ${yt_play[0].url}\n 
 ❏ Sending ${additionalText}, wait a moment．．．`.trim(); 
             ...
             try { 
               ...
               try { 
                 ...
                 try { 
                   ...
                   await conn.reply(m.chat, 'Error, We are trying hard to fix it', m); 
                 } 
               } 
             } 
           } 
     ...
     if (command == 'play2') { 
       try { 
         ...
         await await conn.sendMessage(m.chat, {video: {url: dl_url}, fileName: `${ttl}.mp4`, mimetype: 'video/mp4', caption: `▢ Title: ${ttl}\n Size 𝚅𝙸𝙳𝙴𝙾: ${size}`, thumbnail: await fetch(yt.thumbnail)}, {quoted: m}); 
       } catch { 
         try { 
           const mediaa = await ytMp4(yt_play[0].url); 
           await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `🐝BUMBLEBEE-BOT🐝`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m}); 
         } catch { 
           try { 
 ...
 handler.command = /^(song|play2|play)$/i; 
