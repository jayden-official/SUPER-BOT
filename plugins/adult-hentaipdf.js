...
const handler = async (m, {conn, text, usedPrefix, command, args}) => {
  if (!db.data.chats[m.chat].modohorny && m.isGroup) throw '*[❗𝐈𝐍𝐅𝐎❗] This command can only be used by +18, to enable this feature use the command #enable modohorny*';
  if (!text) throw `*[❗]  Enter the name of the correct category of hentai , example: ${usedPrefix + command} miku*`;
  try {
    m.reply(global.wait);
    ...
    throw `*[❗] error occurred, place choose another category*`;
  }
};
