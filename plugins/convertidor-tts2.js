...
const handler = async (m, { conn, usedPrefix, command, text, args }) => {
  ...
  if (!efecto) {
    ...
    let responseText = `*[❗] You have not entered an effect, please enter a voice effect.*\n\n*—◉ Choose one of the following effects:*\n`;
  ...
  if (!efectoValido) return conn.sendMessage(m.chat, { text: `*[❗] The given effect does not exist in the list, use ${usedPrefix + command} to find out the list of effects.*` }, { quoted: m });

  if (!texto) return conn.sendMessage(m.chat, {text: `*[❗] Enter the text you want to convert to audio.*\n\n*—◉ Example:*\n*◉ ${usedPrefix + command} ${effect} Hello, this is an example of using the command.*`}, { quoted: m});
