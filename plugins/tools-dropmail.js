  ...
  if (lister.includes(feature)) {
    if (feature == 'create') {
      try {
        ...
        conn.dropmail[id] = [
          await m.reply('*EMAIL:*\n' + eml[0] + '\n\n' + '*ID:*\n' + eml[1] + '\n\n*Expired:*\n' + msToTime(timeDiff) + '\n\n_Example *' + usedPrefix + command + ' message* To check the inbox_'),
          eml[0],
          eml[1],
          eml[2],
        ];
    ...
    if (feature == 'message') {
      if (!conn.dropmail[id]) return m.reply('No messages, create an email first\nExample *' + usedPrefix + command + ' create*');

      try {
        const eml = await get_mails(conn.dropmail[id][2]);
        const teks = eml[0].map((v, index) => {
          return `*EMAIL [ ${index + 1} ]*
*From* : ${v.fromAddr} *To* : ${v.toAddr} *Message* : ${v.text} *Size* : ${formatSize(v.rawSize)} *Header* : ${v.headerSubject}
   ...
   `.trim();
    ...
    if (feature == 'delete') {
      ...
      try {
        ...
        await m.reply('Email successfully deleted');
      } catch (e) {
        await m.reply(eror);
      }
    }
