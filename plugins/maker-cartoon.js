...
let handler = async (m, { conn, usedPrefix, command }) => {
	...
	if (m.sender in conn.cartoon)
		throw "https://github.com/jayden-official There is still an unfinished process, my friend. Please wait until it's over. >//<";
	let q = m.quoted ? m.quoted : m;
	let mime = (q.msg || q).mimetype || q.mediaType || "";
	if (!mime) throw `https://github.com/jayden-official Where is the picture you want to convert to a cartoon?`;
		...
		Cartoon(img).then(async (response) => {
			if (response.message == "success") {
				await conn.sendFile(
					...
					"",
					"https://github.com/jayden-official The operation was successful♥  >//<",
					m
				);
				...
				m.reply(
					"https://github.com/jayden-official Excuse me my friend, the picture does not reveal a face, please send a picture in which the face is exposed and visible."
				);
			}
		});
	} catch {
		m.reply("https://github.com/jayden-official Process failed :(");
	} finally {
