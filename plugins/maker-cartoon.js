...
let handler = async (m, { conn, usedPrefix, command }) => {
	...
	if (m.sender in conn.cartoon)
		throw "https://github.com/Khalid-official There is still an unfinished process, my friend. Please wait until it's over. >//<";
	let q = m.quoted ? m.quoted : m;
	let mime = (q.msg || q).mimetype || q.mediaType || "";
	if (!mime) throw `https://github.com/Khalid-official Where is the picture you want to convert to a cartoon?`;
	...
	try {
		Cartoon(img).then(async (response) => {
			if (response.message == "success") {
				await conn.sendFile(
					...
					"",
					"https://github.com/Khalid-official The operation was successful♥  >//<",
					m
				);
		...
		m.reply("https://github.com/Khalid-official Process failed :(");
	} finally {
