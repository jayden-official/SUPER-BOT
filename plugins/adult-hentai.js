...
const handler = async (m, {conn, text, usedPrefix, command, args}) => {
  if (!db.data.chats[m.chat].modohorny && m.isGroup) throw '*[❗𝐈𝐍𝐅𝐎❗] The +18 commands are disabled in this group if you are admin and want to activate them use the command #enable modohorny*';
  if (!text) throw `*[❗] Ingress the name of some hentai categories, for example: ${usedPrefix + command} miku*`;
  try {
    m.reply(global.wait);
