...
let handler = async (m, { conn, usedPrefix, args}) => {
	let towidth = args[0]
	let toheight = args[1]
	if (!towidth) throw '*What size width do I put?*' if (!toheight) throw '*How long do I put?*'
	
let q = m.quoted ? m.quoted : m
let mime = (q.msg || q).mimetype || ''
if (!mime) throw `⚠️ *_Remember to tag an image._*\n*📌 Example:* ${usedPrefix + command} 400 400`

let media = await q.download()
let isMedia = /image\/(png|jpe?g|gif)|video\/mp4/.test(mime)
if (!isMedia) throw `⚠️ *The file ${mime} is not supported._*`
