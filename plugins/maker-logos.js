...
const handler = async (m, {conn, args: [effect], text: txt, usedPrefix, command, name}) => {
  ...
  const res = await maker(effectoSelect?.url, [...text]).catch(_ => { throw '*[❗] we kijana weka command vizuri mbwa*' })
   if (typeof res == 'number') throw res == -1 ? `*[❗𝐈𝐍𝐅𝐎❗] 𝘁𝗵𝗲 𝗲𝗳𝗳𝗲𝗰𝘁 ${effect}  𝗶𝘀 𝗻𝗼𝘁 𝗳𝗼𝘂𝗻𝗱 𝗼𝗻 𝘁𝗵𝗲 𝗹𝗶𝘀𝘁 𝗼𝗳 𝗲𝗳𝗳𝗲𝗰𝘁𝘀*` : `*[❗𝐈𝐍𝐅𝐎❗] 𝗧𝗵𝗲 𝗰𝗼𝗿𝗿𝗲𝗰𝘁 𝘂𝘀𝗲 𝗼𝗳 𝗰𝗼𝗺𝗮𝗻𝗱 𝗶𝘀 ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`;
