...
const handler = async (m, {conn, text, participants, args}) => {
  ...
  try {
    ...
    for (const user of participant.filter((item) => item.attrs.error == 403)) {
      ...
      const teks = `*[❗𝐈𝐍𝐅𝐎❗] IT WAS NOT POSSIBLE TO ADD @${jid.split('@')[0]}, THIS MAY HAPPEN BECAUSE THE NUMBER IS INCORRECT, THE PERSON HAS RECENTLY LEFT THE GROUP OR THE PERSON HAS CONFIGURED THEIR GROUP PRIVACY, THE INVITATION TO THE GROUP WAS SENT TO THE USER IN THEIR PROVIDED*`;
    ...
    throw '*[❗𝐈𝐍𝐅𝐎❗] IT IS NOT POSSIBLE TO ADD THE NUMBER THAT YOU ENTERED, THIS MAY OCCUR BECAUSE THE NUMBER IS INCORRECT, THE PERSON HAS RECENTLY LEFT THE GROUP OR THE PERSON HAS CONFIGURED THEIR GROUP PRIVACY, WE ADVISE YOU SEND THE INVITATION MANUALLY!!*';
  }
};
