let handler = async(m, { conn, text }) => {
  ...
  if (!symbolRegex.test(text)) {
    throw `https://github.com/Khalid-official *[❗] Invalid symbol input. Please provide exactly one symbol as a prefix.*`
  }

  // If the input is valid (contains exactly one symbol), update the prefix
  global.prefix = new RegExp('^[' + text.replace(/[|\\{}()[\]^$+*?.\-\^]/g, '\\$&') + ']')
  await m.reply(`https://github.com/Khalid-official *[❗] the prefix has been changed to* *${text}*`)
}
